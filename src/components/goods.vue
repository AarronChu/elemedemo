<template>
  <div id="goods">
    ...
    <div class="menu-wrapper" ref="menuWrapper">
      <ul>
        <li v-for="(item,index) in goods" :key="index" class="menu-item" :class="{current: currentIndex === index}" @click="selectIndex(index, $event)">
    ...
    <div class="foods-wrapper" ref="foodWrapper">
      <ul>
        <li v-for="(items, index) in goods" :key="index" class="foods-list-hook">
          ...
          <ul>
            <li v-for="(item2, index) in items.foods" :key="index" class="items-content-wrapper" @click="showFood(item2,$event)">
              ...
              <div class="items-content border-1px">
                ...
                <div class="prices">
                  <span class="price-icon">￥</span><span class="price">{{item2.price}}</span>
                  <span v-if="item2.oldPrice" class="old-price" style="margin-left:8px">￥{{item2.oldPrice}}</span>
                </div>
                <div class="cart-control-wrapper">
                  <cart-control :food=item2></cart-control>
                </div>
              </div>
            </li>
          </ul>
    ...
    <food-info :food="foodToShow" :ratings="foodToShow.ratings" ref="foodInfo"></food-info>
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    axios.get('/good/goods').then(res => {
      if(res.data.code === 0){
        ...
        Vue.nextTick(() => {
          this._initscroll() // dom 绑定一个scroll
          this._calculateHeight() // 计算下一个foods高度
        })
      }
    })
  },
  ...
  methods: {
    showFood (food, $event) {
      this.foodToShow = food
      // this.foodShow = !this.foodShow
      this.$refs.foodInfo.toggleFood()
    },
